...
def get_bams_and_beds(ref_seq_file_path: str, input_fastq_file_path:str) -> str:
    ...
    os.system("samtools view -F 2304 -b {}.sam > {}.bam".format(output_file, output_file))
...
def main():
    ...
    for read_name in reads.keys():
        ...
        if len(reads[read_name]) < args.low_length_limit or len(reads[read_name]) > args.upper_length_limit:
            print("skipping read {} for inaproppriate length".format(read_name))
            continue
        ...
        res = map_to_correct_region(start, genomic_regions)
        
        if res is None:
            print("skipping read {}, does not match well any of the pre-defined regions".format(read_name))
            continue
