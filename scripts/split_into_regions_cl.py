...
def main(): 
    ...
    parser.add_argument('--in_path', type=str, help='Path to input fastq file')
    parser.add_argument('--out_path', type=str, help='Path to output fastq file')
    parser.add_argument('--ref_seq', type=str, required=False, default="/Users/ioanna/CliqueSNV/LUMC/ref/SARS-CoV-2-NC_045513.fasta", help='Path to reference sequence')
    ...
    os.system("minimap2 -ax map-pb {} {}.fastq > {}.sam".format(ref_seq, input_path, out_path))  
    os.system("samtools view -bS {}.sam > {}.bam".format(out_path, out_path))
    os.system("samtools sort {}.bam -o {}.sorted.bam".format(out_path, out_path))
    os.system("samtools index {}.sorted.bam".format(out_path))
    os.system("bedtools bamtobed -i {}.sorted.bam > {}.bed".format(out_path, out_path))

    # map reads to genomic regions
    # read bed file with pandas
    df = pd.read_csv(out_path + ".bed", sep="\t", header=None)
